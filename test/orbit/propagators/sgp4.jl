...
@testset "Tests from the paper AIAA 2006-6753" begin
    ...
    let
        ...
        for tle in tles_test_01
            ...
            @inbounds for k = 1:length(t)
                @test eltype(r_TEME[k]) === Float64
                @test eltype(v_TEME[k]) === Float64
    ...
    let
        ...
        for tle in tles_test_01
            ...
            # Compare the results.
            @inbounds for k = 1:length(t)
                @test eltype(r_TEME[k]) === Float32
                @test eltype(v_TEME[k]) === Float32
                ...
                # Compare the values.
                @test st_sgp4_result[1] == SGP4_results[k,1]
                @test st_sgp4_result[2] ≈  SGP4_results[k,2] atol=7e-0
                @test st_sgp4_result[3] ≈  SGP4_results[k,3] atol=7e-0
                @test st_sgp4_result[4] ≈  SGP4_results[k,4] atol=7e-0
                @test st_sgp4_result[5] ≈  SGP4_results[k,5] atol=3e-3
                @test st_sgp4_result[6] ≈  SGP4_results[k,6] atol=3e-3
                @test st_sgp4_result[7] ≈  SGP4_results[k,7] atol=3e-3
            end
        end
        ...
        @inbounds for k = 1:length(t)
            ...
            # conversion from Julian Day to seconds.
            @test st_sgp4_result[1] == SGP4_results[k,1]
            @test st_sgp4_result[2] ≈  SGP4_results[k,2] atol=5e-1
            @test st_sgp4_result[3] ≈  SGP4_results[k,3] atol=5e-1
            @test st_sgp4_result[4] ≈  SGP4_results[k,4] atol=5e-1
            @test st_sgp4_result[5] ≈  SGP4_results[k,5] atol=5e-4
            @test st_sgp4_result[6] ≈  SGP4_results[k,6] atol=5e-4
            @test st_sgp4_result[7] ≈  SGP4_results[k,7] atol=5e-4
        end
    end
end
