...
@testset "Function rECEFtoECI PEF => TOD" begin
    ...
    D_TOD_PEF = rECEFtoECI(PEF(), TOD(), JD_UT1)
    r_tod = D_TOD_PEF*r_pef
    ...
    q_TOD_PEF = rECEFtoECI(Quaternion, PEF(), TOD(), JD_UT1)
    r_tod = vect(conj(q_TOD_PEF)*r_pef*q_TOD_PEF)
...
@testset "Function rECEFtoECI ITRF => ERS" begin
    JD_UTC = DatetoJD(2004, 4, 6, 7, 51, 28.386009)
    r_itrf = [-1033.4793830; 7901.2952754; 6380.3565958]

    D_ERS_ITRF = rECEFtoECI(ITRF(), ERS(), JD_UTC, eop_iau2000a)
    ...
    q_ERS_ITRF = rECEFtoECI(Quaternion, ITRF(), ERS(), JD_UTC, eop_iau2000a)
...
@testset "Function rECEFtoECI ITRF => MJ2000" begin
    JD_UTC = DatetoJD(2004, 4, 6, 7, 51, 28.386009)
    r_itrf = [-1033.4793830; 7901.2952754; 6380.3565958]

    D_MJ2000_ITRF = rECEFtoECI(ITRF(), MJ2000(), JD_UTC, eop_iau2000a)
    ...
    q_MJ2000_ITRF = rECEFtoECI(Quaternion, ITRF(), MJ2000(), JD_UTC, eop_iau2000a)
...
@testset "Function rECEFtoECI TIRS => ERS" begin
    ...
    D_ERS_TIRS = rECEFtoECI(TIRS(), ERS(), JD_UTC, eop_iau2000a)
    ...
    q_ERS_TIRS = rECEFtoECI(Quaternion, TIRS(), ERS(), JD_UTC, eop_iau2000a)
...
@testset "Function rECEFtoECI TIRS => MJ2000" begin
    JD_UTC = DatetoJD(2004, 4, 6, 7, 51, 28.386009)
    r_tirs = [-1033.47503120; 7901.30558560; 6380.34453270]

    D_MJ2000_TIRS = rECEFtoECI(TIRS(), MJ2000(), JD_UTC, eop_iau2000a)
    ...
    q_MJ2000_TIRS = rECEFtoECI(Quaternion, TIRS(), MJ2000(), JD_UTC, eop_iau2000a)
